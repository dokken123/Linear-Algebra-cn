...
def check_vector_span(set_of_vectors, vector_to_check):
    ...
    try:
        ...
        vector_of_scalars = linalg.solve(set_of_vectors, vector_to_check)
...
# 3 TODO
v_a = np.array([[7,6],[29,3]])
v_b = np.array([[2,-8],[9,10]])
v_c = np.array([[2,17],[1,5]])

v_abc = v_a * v_b * v_c
v_cab = v_c * v_a * v_b
v_bca = v_b * v_c * v_a

t_abc = np.trace(v_abc)
t_cab = np.trace(v_cab)
t_bca = np.trace(v_bca)

print(t_abc == t_cab == t_bca)
...
def calc_Norm(x, p = 2, infty = False):
    ...
    if p == 1:
        sum_result = 0
        for val in np.ndenumerate(x):
            sum_result += abs(float(val[-1]))
        return sum_result
    elif p == 2:
        sum_result = 0
        for val in np.ndenumerate(x):
            sum_result += float(val[-1]) ** 2
...
def calc_Condition_Number(A):
    A_M = np.matrix(A)
    print(A_M)
    A_F = calc_Frobenius_Norm(A_M)
    A_I_F = calc_Frobenius_Norm(A_M.I)
    return A_F * A_I_F
...
def calc_svd(A):
    A_M = np.matrix(A)
    svd = np.linalg.svd(A_M)
    print(svd)
    print(np.shape(svd[0]))
    print(np.shape(svd[1]))
    print(np.shape(svd[2]))
    return svd
...
c_img = img.convert("L")
c_arr = np.array(c_img.getdata(band=0))

cs_img = c_arr.reshape(img.size[1],img.size[0])

U, D, VT = calc_svd(cs_img)
